...
<p><code>config/vm.args.src</code> sets <code>-name ${ASOBI_NODE_NAME}@${ASOBI_NODE_HOST}</code> and
...
<p><strong>Running the <a href="https://hexdocs.pm/asobi/datagram-plane.html">datagram gateway</a> makes this sharper, and the
localhost bind below does not answer it.</strong> The two roles share a network
...
roles <strong>different</strong> <code>ERLANG_COOKIE</code>s, or a bug in that codec is <code>rpc:call</code> into
the node holding your Lua sandbox and your database credentials. The compose in
the self-hosting guide shows both.</p>
...
model](https://asobi.dev/docs/security/threat-model) and [Auth and rate
limiting](https://asobi.dev/docs/security/auth). This page is the complement: what the runtime does
...
model](https://asobi.dev/docs/security/lua-sandbox).
...
**Running the [datagram gateway](https://hexdocs.pm/asobi/datagram-plane.html) makes this sharper, and the
...
distribution. See [Threat model](https://asobi.dev/docs/security/threat-model#erlang-distribution).
...
limiting](https://asobi.dev/docs/security/auth)), and Lua callbacks are separately bounded by a
wall-clock timeout, a heap cap and a reduction budget (see [Sandbox
model](https://asobi.dev/docs/security/lua-sandbox#per-callback-budgets)). What is not bounded is
trusted in-VM Erlang code: a game module, a plugin or a NIF gets no reduction
count, no heap cap and no scheduler quota. Enforcement for those is at the OS
or container layer:
...
result next to them. Add these flags to however you already run the image, on
top of the database variables from [Self-hosting](https://hexdocs.pm/asobi/self-hosting.html):
...
- [Threat model](https://asobi.dev/docs/security/threat-model) - the trust assumptions these limits follow from.
- [Auth and rate limiting](https://asobi.dev/docs/security/auth) - the per-request bounds that do exist.
- [Sandbox model](https://asobi.dev/docs/security/lua-sandbox) - what the Lua sandbox removes, replaces and bounds.
- [Trust model](https://asobi.dev/docs/security/lua-trust-model) - what that sandbox is and is not a boundary against.
