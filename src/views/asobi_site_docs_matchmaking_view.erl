...
nodes never match each other, and a restart drops every waiting ticket. Behind a
load balancer this is the fact that decides whether matchmaking works at all;
see [Clustering](https://asobi.dev/docs/clustering).
...
callback runs mid-match, so it has to cope with a player arriving into a live
game state, and the script decides when to stop taking them with
[`game.match.set_joinable(false)`](https://hexdocs.pm/asobi/lua-api.html#match). See
[Lobbies](https://hexdocs.pm/asobi/lobbies.html#joining-a-match-already-in-progress).

## Playing with friends

Gathering players before a game starts is covered in [Lobbies](https://hexdocs.pm/asobi/lobbies.html).
...
reads the queue and cannot act on it - there is no cancel-ticket button, and the
numbers are this node's queue only. See [Operator console](https://hexdocs.pm/asobi/console.html).

## Next steps

- [Testing with multiple players](https://asobi.dev/docs/tools/multiple-players) - why your two
  test clients are one player, and why they land in separate matches.
- [WebSocket protocol](https://asobi.dev/docs/protocols/websocket) - the `matchmaker.*` and `match.*` frames.
- [Configuration](https://asobi.dev/docs/configuration) - per-mode matchmaker tuning.
- [Clustering](https://asobi.dev/docs/clustering) - what a second node does to the queue.
""".
