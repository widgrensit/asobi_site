...
than only in the full list. <a href="/docs/clustering">Clustering</a> has the rest.</p>
...
with guides and demos is in the <a href="https://github.com/widgrensit/asobi/blob/main/README.md#client-sdks">README</a>.</p>
...
the replacement for a Nakama RPC, a PlayFab CloudScript function and a Hathora
custom message. See [Extensions](https://hexdocs.pm/asobi/extensions.html).
...
connections. Each connection costs ~13-20KB, so at that concurrency the ceiling
is CPU spent on message processing, not memory. Figures and method are in
[Benchmarks](https://hexdocs.pm/asobi/benchmarks.html).
...
- You prefer self-hosted Apache-2.0 over a closed managed cloud, with a real
  exit runbook (see [Exit guarantee](https://hexdocs.pm/asobi/exit.html)).
...
- You need a fully managed cloud at hyperscaler breadth. asobi's managed
  ...
  open-source core rather than a different product - invite-only today, and
  narrower than self-hosting in ways [Cloud](https://hexdocs.pm/asobi/cloud.html) lists.
...
loses their world with no error. That last one is the only item here a player
notices and an operator does not, which is why it belongs in the summary rather
than only in the full list. [Clustering](https://asobi.dev/docs/clustering) has the rest.
...
top of the Dart SDK rather than an eighth protocol implementation. The table
with guides and demos is in the [README](https://github.com/widgrensit/asobi/blob/main/README.md#client-sdks).

## Migrating from another backend

- [From Hathora](https://hexdocs.pm/asobi/migrate-from-hathora.html) - shutdown 2026-05-05
- [From PlayFab](https://hexdocs.pm/asobi/migrate-from-playfab.html)
- [From Nakama self-host](https://hexdocs.pm/asobi/migrate-from-nakama.html)
""".
