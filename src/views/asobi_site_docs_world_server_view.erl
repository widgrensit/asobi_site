...
large-scale survival. For arena-style games with smaller player counts, use the
[match server](https://asobi.dev/docs/matchmaking).

For massive tile-based worlds, see [Large worlds](https://hexdocs.pm/asobi/large-worlds.html) for lazy zone
...
carries the same work. Without this a slow zone accumulates ticks without
bound until the node runs out of memory. Each skip increments
`[asobi, zone, tick_skipped]`; see [Observability](https://hexdocs.pm/asobi/observability.html#the-events)
...
| `phases(config)` | no | See [Phases](https://asobi.dev/docs/phases) |
| `on_phase_started(name, state)` / `on_phase_ended(name, state)` | no | Phase transitions |
| `on_zone_loaded(cx, cy, state)` / `on_zone_unloaded(cx, cy, state)` | no | See [Large worlds](https://hexdocs.pm/asobi/large-worlds.html) |
| `terrain_provider(config)` | no | See [Large worlds](https://hexdocs.pm/asobi/large-worlds.html) |
...
[Voting](https://asobi.dev/docs/voting) for what does and does not reach a Lua world today.
...
server does read it every tick. It does not start a vote today: the decoded
table reaches the vote server with the wrong key type and the failure is
swallowed. [Voting](https://asobi.dev/docs/voting) has the detail and the Erlang route that works.
...
Using a world as a persistent hub is covered in [Lobbies](https://hexdocs.pm/asobi/lobbies.html).
...
|---|---|
| Active zones per world | 10,000. See [Large worlds](https://hexdocs.pm/asobi/large-worlds.html) for what happens at that ceiling |
...
See [Configuration](https://asobi.dev/docs/configuration) for the `rehome` rate limit on how often
a player may re-home at all. NPCs re-home directly without going through that
...
[WebSocket protocol](https://asobi.dev/docs/protocols/websocket) for the envelope.
...
| `world.ack` | `{tick, seq}` | Per-connection input ack for prediction; see [Client-side prediction](https://asobi.dev/docs/protocols/websocket#client-side-prediction) |
| `world.phase_changed` | the phase info block | See [Phases](https://asobi.dev/docs/phases) |
| `world.terrain` | `{coords, data}` | See [Large worlds](https://hexdocs.pm/asobi/large-worlds.html) |
...
world is the thing that is full, shard it in your game design - regions,
instances, shards. See [Clustering](https://asobi.dev/docs/clustering).
...
console nor the ops API - see [Operator console](https://hexdocs.pm/asobi/console.html).

## Next steps

- [Large worlds](https://hexdocs.pm/asobi/large-worlds.html) - lazy zones, terrain, and the zone ceiling.
- [Lua scripting](https://asobi.dev/docs/lua/api) - the match-side scripting model.
- [Voting](https://asobi.dev/docs/voting) - in-session voting.
- [Phases](https://asobi.dev/docs/phases) - the phase clock and `world.phase_changed`.
- [Clustering](https://asobi.dev/docs/clustering) - multi-node deployment.
""".
