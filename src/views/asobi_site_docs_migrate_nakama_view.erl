...
| RPC endpoints | Extension RPC over the WebSocket | Frame `rpc.call` with `{protocol: 1, method, params}`; replies `rpc.ok` `{result}` or `rpc.error` `{error: {code, message, details}}`, correlated by `cid`. All seven client SDKs support it. See [Extensions](https://hexdocs.pm/asobi/extensions.html). |
...
| WebSocket | `/ws`, `session.connect` first frame | See the Hathora guide's [WebSocket handshake](https://hexdocs.pm/asobi/migrate-from-hathora.html#websocket-handshake). |
...
on - see Phase 2 below, or [Getting started](https://hexdocs.pm/asobi/getting-started.html) for a complete
...
- Anything a client must call by name, that is not tied to a match, becomes an
  ...
  WebSocket. See [Extensions](https://hexdocs.pm/asobi/extensions.html).
...
is per node, so players queuing against different nodes never match each other,
and rate limits are per node. Adding nodes is not free of design consequences.
[Clustering](https://asobi.dev/docs/clustering) has the full list.
