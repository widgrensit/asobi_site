...
and demos in the <a href="https://github.com/widgrensit/asobi/blob/main/README.md#client-sdks">README</a>.</p>
...
and demos in the [README](https://github.com/widgrensit/asobi/blob/main/README.md#client-sdks).
...
platform sign-in, IAP receipt verification, extensions and runtime tuning.
Everything about the game itself - callbacks, protocol, error codes - is
identical. [Cloud](https://hexdocs.pm/asobi/cloud.html) has the full list of what each side gets.

If it disappears, the open-source node above is enough to run your game
forever. See [exit.md](https://hexdocs.pm/asobi/exit.html).
...
- **Console** - the operator UI this node serves at `/console`. Off until
  `console` is set. See [Operator console](https://hexdocs.pm/asobi/console.html).
...
- **Erasure** - deleting a player and everything core holds about them, in one
  ...
  `POST /api/v1/ops/players/:id/erase`. See [REST API](https://asobi.dev/docs/protocols/rest).
- **Extension** - an OTP application that depends on asobi, added to your
  release, declaring a manifest. It can add RPC methods, workers, schemas and
  ops actions without forking asobi. See [Extensions](https://hexdocs.pm/asobi/extensions.html).
...
- **Tenant** - a studio or account in the managed cloud. Not a concept when
  self-hosting. See [Cloud](https://hexdocs.pm/asobi/cloud.html).
...
asobi uses the first. The [Nakama](https://hexdocs.pm/asobi/migrate-from-nakama.html),
[PlayFab](https://hexdocs.pm/asobi/migrate-from-playfab.html) and [Hathora](https://hexdocs.pm/asobi/migrate-from-hathora.html)
