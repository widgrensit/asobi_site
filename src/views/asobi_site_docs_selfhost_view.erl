    ...
    Content = ?html({'div', [], [
        ...
        {'div', [{class, ~"docs-grid"}], [
            {a, [{href, ~"#docker-compose"}, {class, ~"docs-card"}], [
                {h3, [], [~"Docker Compose"]},
                {p, [], [~"Single machine, everything local. Great for dev, prototyping, and small productions."]}
            ]},
            {a, [{href, ~"#single-server"}, {class, ~"docs-card"}], [
                {h3, [], [~"Single VPS"]},
                {p, [], [~"One Hetzner/DigitalOcean box, Erlang release, systemd. Supports hundreds of CCU."]}
            ]},
            {a, [{href, ~"#kubernetes"}, {class, ~"docs-card"}], [
                {h3, [], [~"Kubernetes"]},
                {p, [], [~"Scaleway Kapsule or similar. Container-per-game-env, full isolation, scale horizontally."]}
            ]}
        ]},
...
"""),
        {p, [], [
            ...
            {a, [{href, ~"https://github.com/widgrensit/asobi/tree/main/config"}], [~"the config directory"]},
            ~" for full examples."
        ]},
...
"""),
        ...
        {p, [], [
            ...
            {a, [{href, ~"https://github.com/widgrensit/asobi-infra"}], [~"widgrensit/asobi-infra"]},
...
"""),
        ...
        {h2, [], [~"Backups"]},
        {ul, [], [
            {li, [], [{strong, [], [~"Postgres: "]}, ~"everything Asobi persists lives in Postgres. Back up continuously (WAL shipping to S3-compatible storage) or at least nightly."]},
            {li, [], [{strong, [], [~"Lua bundles: "]}, ~"your game source lives in git. That's the backup."]},
            {li, [], [{strong, [], [~"Zone snapshots: "]}, ~"persistent worlds snapshot their state to Postgres every tick interval. They're restored automatically on startup."]}
        ]},
        ...
        {ul, [], [
            ...
            {li, [], [~"Enable TLS in the reverse proxy; don't expose the raw ",
                {code, [], [~"8080"]}, ~" port to the public internet."]},
            {li, [], [~"Enable Postgres SSL for connections from other hosts."]},
            {li, [], [~"Run the engine as a non-root user (the Docker image does this; bare metal should too)."]},
